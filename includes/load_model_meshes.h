...
public:
	...
	DaeModel(const char* model_path) // Constructor
	{
...
private:
	void load_model()
	{
		...
		else
		{
			...
			for (unsigned int i = 0; i < num_meshes; ++i)
			{
				...
				// indices_offset += mesh->mNumVertices; // Disabled for tutorial: DaeModel Loading (Part 1 of 3)
	...
	unsigned int load_texture_image(std::string file_name, bool& load_complete)
	{
		...
		file_name = FileSystem::getPath("resources/textures/dae_models/" + file_name.substr(position + 1));
